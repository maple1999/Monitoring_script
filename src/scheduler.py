from __future__ import annotations

import time
from datetime import datetime, timedelta, timezone
from typing import Callable


def _sgt_now() -> datetime:
    # Asia/Singapore is UTC+8 (no DST)
    return datetime.utcnow().replace(tzinfo=timezone.utc) + timedelta(hours=8)


def next_run_time_sgt(target_hh: int, target_mm: int) -> datetime:
    now = _sgt_now()
    run = now.replace(hour=target_hh, minute=target_mm, second=0, microsecond=0)
    if run <= now:
        run = run + timedelta(days=1)
    return run


def run_daily(target_time_str: str, job: Callable[[], int]):
    hh, mm = map(int, target_time_str.split(":"))
    while True:
        run_at = next_run_time_sgt(hh, mm)
        # compute sleep seconds in UTC terms
        now_utc = datetime.utcnow().replace(tzinfo=timezone.utc)
        run_utc = run_at - timedelta(hours=8)
        secs = (run_utc - now_utc).total_seconds()
        if secs > 0:
            time.sleep(min(secs, 3600))
            # re-loop to correct drift
            continue
        # execute
        try:
            code = job()
        except Exception:
            pass

